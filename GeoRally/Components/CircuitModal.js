                        ...
                        {                            
                            this.props.connected ?
                                <View style={{flexDirection:'row'}}>
                                    <BetterBadderPicker onValueChange={(item) => this.setState({idLanguage: item.id})} style={{ flex: 4, margin: 5 }} keyMember="id" displayMember="label" selected={this.props.marker.supportedLanguages.find(item => item.id === this.state.idLanguage)} items={this.props.marker.supportedLanguages.filter((language) => language.supportedPercentage == 100)} title="Langage" />
