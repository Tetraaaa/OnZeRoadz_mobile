    ...
    componentDidUpdate(prevProps)
    {
        ...
        if ((prevProps.marker != null && this.props.marker!= null && prevProps.marker.id != this.props.marker.id) || (!prevProps.marker && this.props.marker)){            
            if(this.props.playable){
                ...
                this.setState({
                    idLanguage: this.props.marker.supportedLanguages[0].language.id
                });
            }            
        }
    }
                        ...
                        {                            
                            this.props.connected ?
                                <View style={{flexDirection:'row'}}>
                                    {this.props.playable ? <Flag style={{ flex: 2, margin: 5, padding:5 }} size={32} code={this.props.marker.language.code.toString().toUpperCase()} />  : <BetterBadderPicker onValueChange={(item) => this.setState({idLanguage: item.language.id})} style={{ flex: 4, margin: 5 }} keyMember="id" displayMember="label" selected={this.props.marker.supportedLanguages.find(item => item.language.id === this.state.idLanguage)} items={this.props.marker.supportedLanguages.filter((language) => language.supportedPercentage == 100)} title={Strings("circuitModal", "language")} />}
